...
//this player uses desirability function 1 with alex's estimator
//TODO
public class EstimatingPlayer3 implements GinRummyPlayer {
	...
	int turn;
	public static final float OPPO_CARD_PROB_WEIGHT = 0.5f;
		...
		for(int i = 0; i < candidates.size(); i++) {
			...
			for(int j = 0; j < Card.NUM_CARDS; j++) {
				...
				for(Long meldBitstring : GinRummyUtil.getAllMeldBitstrings()) {
					...
					if(cards.contains(Card.getCard(i)) && cards.contains(Card.getCard(j)))
						numOfMeld++;
				}
